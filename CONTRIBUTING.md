...
- **Remote MCP setup** — MCP servers must be deployed as remote HTTP endpoints and connected via custom connectors (URL-based). New work should use the Convex HTTP action pattern in [`integrations/convex-open-brain`](integrations/convex-open-brain/). Supabase Edge Functions remain valid only for legacy validation. Do NOT use local Node.js servers or `claude_desktop_config.json`.
...
14. **Remote MCP pattern** — Extensions and integrations must use remote MCP over HTTP. New work should target Convex HTTP actions. No `claude_desktop_config.json`, no local Node.js stdio servers. See the [Getting Started guide](docs/01-getting-started.md) for the correct pattern
